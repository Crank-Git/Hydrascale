...
// State represents the tailscaled state file contents.
type State struct {
...
func StartDaemon(tailnetID string, namespaceName string) error {
	...
	fmt.Printf("tailscaled started in namespace %q (PID %d)\n", namespaceName, pid)
	return nil
}
...
func StopDaemon(namespaceName string, tailnetID string) error {
	...
	for {
		...
		case <-deadline:
			...
			os.Remove(pidPath)
			fmt.Printf("tailscaled for %s force-killed (PID %d)\n", tailnetID, pid)
			return nil
		case <-tick.C:
			...
			if err := proc.Signal(syscall.Signal(0)); err != nil {
				// Process is gone
				os.Remove(pidPath)
				fmt.Printf("tailscaled for %s stopped (PID %d)\n", tailnetID, pid)
				return nil
			}
		}
	}
...
// readState reads a tailscaled state file.
...
// PollState polls the tailscaled state file at the given interval.
func PollState(pollInterval time.Duration) (<-chan *State, error) {
	...
	go func() {
		...
		defer ticker.Stop()
		for range ticker.C {
			state, err := readState(filepath.Join(DefaultStateDir, "tailscaled.state"))
