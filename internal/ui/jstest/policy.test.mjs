...
test("the section nav marks the selected section, and the Rules row opens nothing", () => {
  const markup = visualMarkup(sectionsBody(), "groups");
  assert.match(markup, /data-nav="groups" aria-selected="true"/);
  assert.match(markup, /data-nav="hosts" aria-selected="false"/);
  assert.ok(!markup.includes('data-nav="rules"'));
});

test("the section nav draws no entry list before the operator selects a section", () => {
  assert.ok(!visualMarkup(sectionsBody()).includes("setlist"));
});
