...
export function visualMarkup(sections, nav = "", pendingRemoval = null) {
  ...
  const opaque = sections.opaque_keys && sections.opaque_keys.length
    ...
    : "";
  // Rules is the home section: an unset nav (the empty string) draws the matrix, the
  // same as a nav explicitly set to "rules", so the matrix stays the view the operator
  // reaches before any nav row is clicked. #316 owns the matrix and the rule list it
  // opens; #315 owns the four named-set entry lists.
  const activeNav = nav || "rules";
  const body = activeNav === "rules"
    ? matrixMarkup(matrixModel(sections))
