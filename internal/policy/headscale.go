...
// A write needs the database policy mode. cmd/headscale/cli/policy.go:114 of the
// Headscale source at tag v0.29.3 states the constraint.
var ErrHeadscaleFileMode = errors.New(`the Headscale control server runs the file policy mode, and a policy write needs policy.mode: "db"`)
...
// Read works in both policy modes. Read returns ErrHeadscaleNoPolicyRoute when the
// control server exposes no policy route, and it returns the transport error unchanged
// when the request does not reach the control server.
