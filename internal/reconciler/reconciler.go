...
// SetEventLog opens a file for JSON event logging in append mode.
func (r *Reconciler) SetEventLog(path string) error {
...
// Close closes the event log file if open.
func (r *Reconciler) Close() error {
	if r.eventFile != nil {
		err := r.eventFile.Close()
		r.eventFile = nil
		return err
	}
	return nil
...
func (r *Reconciler) Shutdown() error {
	...
	var wg sync.WaitGroup
	for id, state := range actual {
		if !state.DaemonHealthy && !state.NsExists {
			continue
		}
		wg.Add(1)
		go func(tailnetID string, ts *TailnetState) {
