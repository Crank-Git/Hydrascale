...
| 2026-08-05 | 1 | Issue #178. **The operator decided: measured reachability, probed on each tick.** `GET /api/status` now carries `measured_reachability` for each namespace, with the states `reachable`, `unreachable`, and `not_probed`. `healthy` keeps its meaning, which is that the `tailscaled` process runs. The daemon derives the new field from no rule, because a rule that is present is not a packet that arrives. |
| 2026-08-05 | 1 | Issue #178. **The target is the default gateway of the host, and `probe_target` overrides it.** `reach.Prober` runs `ip netns exec <ns> ping -n -c 1 -W 1 <target>`. The gateway sits beyond the host, therefore the packet crosses the `FORWARD` chain and the masquerade rule of the forward path, and it stays on the local network. The default sends no packet to a third party. It proves that the namespace reaches the local network through the forward path. It does not prove that the namespace reaches the internet, and it does not prove that the tailnet peers. An operator who wants proof of the internet declares `probe_target: 1.1.1.1`, and accepts one packet to that address for each namespace on each tick. |
| 2026-08-05 | 1 | Issue #178. **The probe times out at 250 ms, the whole step at 400 ms, and a timeout reports `unreachable`.** The reconciler probes every namespace at the same time, therefore the step costs one budget for one namespace and for eight. The specification bounds the tick at 1 second, and 400 ms holds inside that bound. A path that carries nothing inside the budget carries nothing that the operator can use, therefore the field reports `unreachable` and `detail` holds the reason. |
