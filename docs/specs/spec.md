...
| 2026-08-05 | 1 | Issue #135. **The daemon removes the version 0.9 `FORWARD` rules at start.** A host that upgrades keeps the rules that the version it ran wrote, and the deletes in `TeardownVeth` run for a namespace that goes away. `namespaces.RemoveLegacyForwardRules` matches the two exact forms `-i vh<hash> -j ACCEPT` and `-o vh<hash> -m state --state RELATED,ESTABLISHED -j ACCEPT`, so an operator rule that names the same device keeps its place. |
