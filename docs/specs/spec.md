...
| 2026-08-05 | 1 | Epic 8 started. Issue #154 adds `internal/policy` with the credential and the secrets file. **`WriteCredential` creates no directory**, because `SA-23` records that three writers already set the mode of `/etc/hydrascale`, and a fourth writer of that mode is the defect the finding names. The caller creates the directory, and `config.SaveConfig` already does so. `LoadCredential` reads the file on every call, which the behaviour rule of `features/08-upstream-policy.md` requires, and it refuses a file whose mode is wider than `0600`. The error of a parse failure names the path alone, because the message of the YAML parser can quote the line that failed and that line can hold a credential value. |
