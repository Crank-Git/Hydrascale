...
- **FR-vacl-7** — The matrix places two kinds of identity on both axes. The first kind is
  every tag, group, autogroup, and the wildcard `*` that a rule references. The second
  kind is every group, tag owner, and host alias that the document's `groups`,
  `tagOwners`, and `hosts` sections define. An `ipsets` key names no axis, because a rule
  names an IP set as `ipset:<name>`. The matrix draws each axis in the manner of
  **FR-editor-1** to
